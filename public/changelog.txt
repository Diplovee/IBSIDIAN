...
### Fixed
- Browser tab titles now refresh immediately on navigation instead of getting stuck with stale values (for example, lingering as "google")
- Browser tab favicon fallback now uses each site’s own `/favicon.ico` instead of the Google favicon service

### Changed
- Browser address bar placeholder text now reads "Search web or type URL"
...
### Added
- Linux local installer script and documentation for app menu integration (`bun run install:linux-local`)
- In-app update controls in Settings (check, apply, and restart)
- Startup update check prompt plus a blinking update badge with commit-hash tooltip in the editor header
- DevTools toggle shortcut (`Ctrl+Shift+C`) in Electron main window

### Changed
- Clarified menu wording to "Open in new app window" for platform-neutral wording

### Fixed
- Installed app now loads built renderer unless `ELECTRON_RENDERER_URL` is set
- Terminal sessions persist across tab switches by keeping terminal views mounted per pane
