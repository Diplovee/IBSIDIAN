...
export const TabsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const restoreTabs = useCallback((nextTabs: Tab[], nextActiveTabId: string | null = null, nextBrowserGroups: BrowserTabGroup[] = []) => {
    ...
    setActiveTabId(nextActiveTabId && nextTabs.some(tab => tab.id === nextActiveTabId)
      ? nextActiveTabId
      : nextTabs.length > 0
  ...
  const duplicateBrowserGroup = useCallback((groupId: string) => {
    const sourceTabs = tabs.filter(tab => tab.type === 'browser' && tab.groupId === groupId);
    if (sourceTabs.length === 0) return;

    const newGroupId = createBrowserGroup(`${browserGroups.find(g => g.id === groupId)?.name ?? 'Browser Group'} Copy`, browserGroups.find(g => g.id === groupId)?.color);
    setTabs(prev => [
      ...prev,
      ...sourceTabs.map(tab => ({
        ...tab,
